...
#' @keywords DEA, m, graph
#' @export
#' @examples
#' graph1_m_DEA()

graph1_m_DEA <- function(input, output, mseries, B, RTS, ORIENTATION, check, col, legend, print = TRUE) {
  ...
  for (m in 1:length(mseries)) {
    ...
    DEA <- robust_DEA(input, output, m = mseries[m], B, RTS= RTS, ORIENTATION= ORIENTATION)
    for(l in 1:length(check)) {
      meff[m,l] <- length(DEA[DEA>check[l]])
    }
